...
?>
    ...
    <section class="jumbotron text-center">
        ...
        <div class="container">
            <form action="/laba/registrationcheck.php" method="post" class="form-signin">
                ...
                <?php
                if(!empty(@$_COOKIE['error1']))
                {
                    echo '<div id="errors" style="color:red;">' .$_COOKIE['error1']. '</div><hr>';
                    setcookie('error1','');
                }
                if(!empty(@$_COOKIE['success']))
                {
                    echo '<div id="errors" style="color:blue;">Регистрация успешно проведена</div><hr>';
                    setcookie('success','');
                    echo '<a href="authorization.php">Авторизируетесь пожалуйста!</a>';
                }
                ?>
            </form>
        </div>
